...
window.onload=function(){
    var defaultSettings={
        ...
        visual:{
            ...
            camera:{minZoom:175,maxZoom:2400,defaultZoom:450,dragSpeed:3,viewInitialZoom:{gridSphere:450,gridmap:450,towers:900,productspace:450,productspace3D:450,productsphere:360},productFocusZoom:100},
...
function applyTowersTimelineLayout(){
    ...
    for(var i=0;i<particles;i++){
        var point=names[i];
        if(point && point.y<=timelineCurrentYear && countries[point.c]){
...
function renderOverviewPanel(){
    ...
    var total=0;
    $.each(countries,function(i,country){total+=country.exports || 0;});
    metaText+="<h2>全球绿色技术专利</h2>";
    metaText+="<div class='atlasMetric'><span>专利总量</span><strong>"+formatPatentCount(total)+"</strong></div>";
    metaText+="<div class='atlasMetric'><span>国家/地区</span><strong>"+totalCountries+"</strong></div>";
    metaText+="<div class='atlasMetric'><span>技术子类</span><strong>"+totalProducts+"</strong></div>";
    metaText+="<h3>操作焦点</h3><div class='atlasMuted'>选择国家后查看主要绿色技术构成和专利流向；切换到技术空间后可按技术子类定位节点。</div>";
...
function renderCountryPanel(code){
    ...
    var topProducts=topProductEntries(country,6).map(function(row){
        return {
            label:row.id+" - "+row.product.name,
            value:row.value,
    ...
    html+="<div class='atlasMetric'><span>绿色技术专利</span><strong>"+formatPatentCount(country.exports)+"</strong></div>";
    html+="<div class='atlasMetric'><span>点数量</span><strong>"+formatPatentCount(country.particles)+"</strong></div>";
    html+="<div class='atlasMetric'><span>技术覆盖</span><strong>"+Object.keys(country.products || {}).length+"</strong></div>";
    html+="<h3>主要技术子类</h3>";
    html+=renderBars(topProducts,country.exports,function(row){return row.color;});
    if(partnerRows.length){
        var partnerTotal=partnerRows.reduce(function(sum,row){return sum+row.value;},0);
        html+="<h3>主要受让人国/地区</h3>";
...
function renderProductPanel(productId){
    ...
    $.each(countries,function(code,country){
        var value=country.products && country.products[productId];
        if(value){
            countryCount++;
            rows.push({label:country.name,value:value,color:product.color});
        }
        ...
        if(years){
            $.each(years,function(year,count){
                ...
                if(firstYear===null || numericYear<firstYear)firstYear=numericYear;
                if(latestYear===null || numericYear>latestYear)latestYear=numericYear;
                if(numericYear<=timelineCurrentYear)visibleTotal+=numericCount;
            });
        }
    });
    rows.sort(function(a,b){return b.value-a.value;});
    ...
    html+="<div class='atlasMetric'><span>所属门类</span><strong style='color:"+product.color+"'>"+escapePanelHtml(productCategory(product))+"</strong></div>";
    html+="<div class='atlasMetric'><span>全球专利</span><strong>"+formatPatentCount(total)+"</strong></div>";
    html+="<div class='atlasMetric'><span>当前时间累计</span><strong>"+formatPatentCount(visibleTotal)+"</strong></div>";
    html+="<div class='atlasMetric'><span>覆盖国家/地区</span><strong>"+countryCount+"</strong></div>";
    if(firstYear!==null && latestYear!==null){
        html+="<div class='atlasMetric'><span>数据年份</span><strong>"+firstYear+"-"+latestYear+"</strong></div>";
    }
    html+="<h3>主要国家/地区</h3>";
    html+=renderBars(rows.slice(0,6),total,function(){return product.color;});
...
function visibleTechnologyEdges(network,limit){
    ...
    var byNode={};
    var visible={};
    var edges=network.edges.slice().sort(function(a,b){
        return Number(b.value || 0)-Number(a.value || 0);
    });
    $.each(edges,function(i,edge){
        if(!byNode[edge.source])byNode[edge.source]=[];
        if(!byNode[edge.target])byNode[edge.target]=[];
        if(byNode[edge.source].length<limit){
            byNode[edge.source].push(edge);
            visible[edge.source+"|"+edge.target]=edge;
        }
        if(byNode[edge.target].length<limit){
            byNode[edge.target].push(edge);
            visible[edge.source+"|"+edge.target]=edge;
        }
    });
    return $.map(visible,function(edge){return edge;}).sort(function(a,b){
        return Number(b.value || 0)-Number(a.value || 0);
    });
}
...
function switcher(to,reset,incremental){
    ...
    if(currentSetup!==to || reset){
        if(cameraControls && cameraControls.setAutoRotate){
            cameraControls.setAutoRotate(to==="gridSphere" && setting("visual.globe.autoRotateByDefault",true));
        }
